...
대 모델: ChatGPT, Claude, Gemini, GPT-2, GPT-3, GPT-4
...
### 기타 최신 LLM
- RoPE (Rotary Position Embedding) → GPTNeoX, LLaMA 등에서 쓰이는 위치 인코딩
- FlashAttention → 긴 문장도 빠르게 학습/추론 가능
- Mixture of Experts (MoE) → 모델 파라미터는 크지만, 실제 연산은 일부 전문가 레이어만 활성화
- Instruction Tuning + RLHF → 사용자의 지시(Instruction)를 따르는 훈련 + 인간 피드백 기반 보상 학습
- Retriever-augmented generation (RAG) → 외부 지식 DB 검색해서 답변 강화
